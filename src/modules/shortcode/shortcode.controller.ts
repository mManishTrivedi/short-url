import { Controller, Get, Post, Body, Patch, Param, Delete, Logger, Res, Response, ConflictException, UnprocessableEntityException, Redirect, NotFoundException } from '@nestjs/common';
 ...
 async create(@Body() createShortcodeDto: CreateShortcodeDto) {
    ...
    if (createShortcodeDto.shortcode){
      ...
      if (!createShortcodeDto.shortcode.match('^[0-9a-zA-Z_]{4,}$')) {
        throw new UnprocessableEntityException('Invalid Alias. Must be alphanumeric and _ allowed')
      }

      const record = await this.shortcodeService.findOne({shortcode: createShortcodeDto.shortcode});

      if (record) {
        this.logger.error('Requested alias/shortcode is already exist', record);
        throw new ConflictException('Requested alias/shortcode is already exist')
        // res.status(409).json({message: 'requested alias/shortcode is already exist'})
      }
    }
    let record =  await this.shortcodeService.create(createShortcodeDto);
